...
async function buildRS(log) {
  ...
  log.push('RS universe: ' + n500.length);
  const scores = [];
  const BATCH = 10;
  for (let i = 0; i < n500.length; i += BATCH) {
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      scores.push({ symbol: stock.symbol, name: stock.company, sector: stock.sector || 'N/A', rsScore, price: Math.round(price * 100) / 100, wh52: Math.round(high52 * 100) / 100, from_wh: Math.round(((high52 - price) / high52) * 1000) / 10, ema200 });
    }
  ...
  log.push('RS done: ' + scores.length);
  return scores;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const filtered = all.filter(s => s.rs >= rsMin && s.from_wh <= from52Max && s.ema200 && s.price > s.ema200 && s.price >= priceMin).sort((a, b) => b.rs - a.rs).map(s => ({ ...s, mcap: null, pe: null, avg_val: null }));
